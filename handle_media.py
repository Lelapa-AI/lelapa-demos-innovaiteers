...
def process_audio_file(audio_file):
    ...
    genai.configure(api_key=GEMINI_CHAT_API)
    ...
    model = genai.GenerativeModel(
        ...
        system_instruction="""
Analyze the audio thoroughly and determine the details of the car in the audio.
...
If the audio is not that of a car, politely point out that no car was detected in the audio.
And ask then to try send the voicenote again and make themselves clear  

Questions to be answered based on the details of the car in the audio:
...
# """
    ...
    # Create the prompt.
    prompt = "Describe the contents of the audio file and ascertain the information that would be used to generate an insurance quote that is accurate as possible based the images"
...
# """
    ...
    # Create the prompt.
    prompt = "Describe the contents of the image and ascertain the contents that would be used to generate an insurance quote that is accurate as possible based the images"
...
def delete_file(display_name):
    ...
    for file in genai.list_files():
        if file.display_name == display_name:
            sample_file=genai.get_file(file.name)    
            sample_file.delete()



# process_images("/workspaces/lelapa-demos-innovaiteers/test_image2.jpeg")


def process_audio_file(audio_file):
    GEMINI_CHAT_API = "AIzaSyDRF5ylqwU_zjA1eymJQn1EybLdbxuxePc"
    genai.configure(api_key=GEMINI_CHAT_API)
